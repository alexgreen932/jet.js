...
export function _interpolation(tpl) {
// 
// 
  ...
  return tpl.replace(/(?<!\$)\{(.*?)\}/g, (_, raw) => {
      ...
      const resolvedTokens = tokens
        .map(tok => {
          ...
          if (looksResolvableToken(tok)) {
            ...
            // this._update_checker(val, tok);//not need any more let it check whole data as now is virt DOM
    ...
    // this._update_checker(resolvedValue, expr);//not need any more let it check whole data as now is virt DOM
